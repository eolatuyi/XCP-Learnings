...
void vStartSimpleTCPServerTasks( uint16_t usStackSize, UBaseType_t uxPriority )
{
	...
	xTaskCreate( prvConnectionListeningTask, "ServerListener", usStackSize, ( void * ) &XCP_DRIVER_CBLK, uxPriority + 1, NULL );
...
struct xcp_driver_cblk* cblk = pvParameters;
	...
	for( ;; )
	{
		...
		cblk->connected_sock = FreeRTOS_accept( xListeningSocket, &xClient, &xSize );
		configASSERT( cblk->connected_sock != FREERTOS_INVALID_SOCKET );

		/* Spawn a task to handle the connection. */
		xTaskCreate( prvServerConnectionInstance, "EchoServer", usUsedStackSize, ( void * ) cblk->connected_sock, tskIDLE_PRIORITY, NULL );
	}
}
/*-----------------------------------------------------------*/
...
uint8_t *pucRxBuffer;
	...
	if( pucRxBuffer != NULL )
	{
		...
		for( ;; )
		{
			...
			{
				// Process xcp message
                XcpCommand((uint32_t *)(pucRxBuffer+4));
	...
	do
	{
		if( FreeRTOS_recv( xConnectedSocket, (uint8_t *)pucRxBuffer, ipconfigTCP_MSS, 0 ) < 0 )
		{
			break;
		}
