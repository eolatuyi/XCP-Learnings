...
/*****************************************************************************/
 ...
 {
     struct xcp_driver_cblk* cblk = arg;

     for (;;)
     {
         //TODO: Block until the next release time... service task only if an event is pending
         // check any xcp messages to process
         // process xcp message per priority scheme
         // service CTO request
         // send out DTO's
         // complete transmission
         
         // Handle transmit complete signal outside an isr context
         if(cblk->tx_flag)
         {
             cblk->tx_flag = 0;
             xcp_driver_transmit_complete(cblk);
         }
        ...
        if(cblk->rx_flag)
        {
            ...
            cblk->can_frame[0].pui8MsgData = &cblk->receive_buffer.buffer[0];// Set CAN destination buffer

            // Read the message from the CAN.  Message object number 1 is used
            // (which is not the same thing as CAN ID).  The interrupt clearing
            // flag is not set because this interrupt was already cleared in
            // the interrupt handler.
            CANMessageGet(CAN0_BASE, 1, &cblk->can_frame[0], 0);
            ...
            xcp_driver_print_can_frame_info(&cblk->can_frame[0], 1, 0);
            
            // Process xcp message
            XcpCommand(&cblk->receive_buffer.command);
        }
#if defined(XCP_DRIVER_LOOPBACK_TEST_MODE)
        // Precondition: set tx length, buffer value
        if (cblk->test_trigger_tx)
        {
            cblk->can_frame[2].ui32MsgLen    = cblk->transmit_length;
            cblk->can_frame[2].pui8MsgData   = &cblk->transmit_buffer[0];
            //cblk->tx_flag                    = 0;
            xcp_driver_print_can_frame_info(&cblk->can_frame[2], 3, 1);
            CANMessageSet(CAN0_BASE, 3, &cblk->can_frame[2], MSG_OBJ_TYPE_TX);
            
            cblk->test_trigger_tx = 0;
        }
        
        if (cblk->test_rx_flag)
        {
            cblk->can_frame[3].pui8MsgData = &cblk->receive_buffer.buffer[0];// Set CAN destination buffer
            CANMessageGet(CAN0_BASE, 4, &cblk->can_frame[3], 0);
            cblk->test_rx_flag = 0;
            xcp_driver_print_can_frame_info(&cblk->can_frame[3], 4, 0);        
        }
